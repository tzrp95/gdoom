...
static const int BFG_DAMAGE_FREQUENCY			= 333;	// move to projectile defs
static const float BOUNCE_SOUND_MIN_VELOCITY	= 100.0f;
static const float BOUNCE_SOUND_MAX_VELOCITY	= 600.0f;
...
void idProjectile::Restore( idRestoreGame *savefile ) {
	...
	// DentonMod --->
	damageDef = gameLocal.FindEntityDef( spawnArgs.GetString( "def_damage" ) );	// Reinitialize the damage Def
	// <---
}
...
void idProjectile::Think( void ) {
	...
	if ( state != EXPLODED ) { // update & run physics until projectile is not exploded.
	...
	// this is from Lost Mission, not sure if we should keep it here
	// or put it in the braces above , before RunPhysics, which was modified
	// from DentonMod, without the inclusion of D3XP or D3LM
	if ( mTouchTriggers ) {
		TouchTriggers();
	}
...
bool idProjectile::Collide( const trace_t &collision, const idVec3 &velocity ) {
	...
	if ( spawnArgs.GetBool( "impact_damage_effect" ) ) {
		...
		if ( ( ent->IsType( idBrittleFracture::Type ) || ent->IsType( idAnimatedEntity::Type )
			|| ent->IsType( idMoveable::Type ) || ent->IsType( idMoveableItem::Type ) )
			&& ent->spawnArgs.GetBool( "bleed", "1" ) ) {	// This ensures that if an entity does not have bleed key defined, it will be considered true by default
...
void idProjectile::DefaultDamageEffect( idEntity *soundEnt, const idDict &projectileDef, const trace_t &collision, const idVec3 &velocity ) {
	...
	typeName = gameLocal.surfaceTypeNames[ materialType ];
...
void idProjectile::Fizzle( void ) {
	...
	//	gameLocal.particles->SpawnParticles( GetPhysics()->GetOrigin(), vec3_origin, psystem );
	}
	...
	physicsObj.PutToRest();
	BecomeInactive( TH_PHYSICS ); // DentonMod : This causes the physics not to update when it's fizzled
...
void idProjectile::Explode( const trace_t &collision, idEntity *ignore ) {
	...
	if ( spawnArgs.GetVector( "detonation_axis", "", normal ) ) {
		...
		// Added by Clone JCD for setting proper direction of fx.
		GetPhysics()->SetAxis( collision.c.normal.ToMat3() ); 
	}

	GetPhysics()->SetOrigin( collision.endpos + 0.5f * collision.c.normal );
	...
	if ( !projectileFlags.impact_fx_played ) {
		...
		if ( !( fxname && *fxname ) ) {
			...
			if ( damageDef != NULL ) {
				...
				const char *materialType = gameLocal.surfaceTypeNames[ type ];
...
void idSoulCubeMissile::KillTarget( const idVec3 &dir ) {
	...
	if ( enemy.GetEntity() && enemy.GetEntity()->IsType( idActor::Type ) ) {
		...
		act->GetAFPhysics()->SetTimeScale( 0.50 );
...
void idBFGProjectile::Explode( const trace_t &collision, idEntity *ignore ) {
	...
	/*
	// DentonMod : This causes trouble like splash damage not working at all
	if ( ignore == NULL ) {
...
================
*/
idHomingProjectile::idHomingProjectile( void ) {
	enemy			= NULL;
	speed			= 0.0f;
	turn_max		= 0.0f;
	clamp_dist		= 0.0f;
...
================
*/
void idHomingProjectile::Spawn( void ) {
}
...
================
*/
void idHomingProjectile::Think( void ) {
	idVec3		dir;
	idVec3		velocity;
	idVec3		nose;
	idVec3		tmp;
...
/*
================
idHomingProjectile::Event_SetEnemy
================
*/
...
void idDebris::Spawn( void ) {
	...
	smokeFly = NULL;
	smokeFlyTime = 0;
	nextSoundTime = 0;			// DentonMod 
	soundTimeDifference = 0;	// DentonMod
}
...
void idDebris::Save( idSaveGame *savefile ) const {
	...
	savefile->WriteParticle( smokeFly );
	savefile->WriteInt( smokeFlyTime );
	savefile->WriteInt( soundTimeDifference ); // DentonMod: nextSountTime needs no saving
...
void idDebris::Restore( idRestoreGame *savefile ) {
	...
	savefile->ReadParticle( smokeFly );
	savefile->ReadInt( smokeFlyTime );
	savefile->ReadInt( soundTimeDifference ); // DentonMod: nextSoundTime needs no restoring
...
void idDebris::Launch( void ) {
	...
	physicsObj.SetClipMask( MASK_SOLID | CONTENTS_MOVEABLECLIP );
		...
		// Make sure that the linear velocity is added with owner's linear velocity for more accurate physics simulation. 
...
void idDebris::Think( void ) {
	...
	if ( smokeFly && smokeFlyTime && !IsHidden()) { // Emit particles only when visible
		idVec3 dir = -GetPhysics()->GetLinearVelocity();
		dir.Normalize();
		if ( !gameLocal.smokeParticles->EmitSmoke( smokeFly, smokeFlyTime, gameLocal.random.CRandomFloat(), GetPhysics()->GetOrigin(), dir.ToMat3(), timeGroup  ) ) {
			if ( continuousSmoke ) {

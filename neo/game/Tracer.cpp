...
void dnTracer::Think( void )  {
	...
	owner->GetRenderEntity()->origin	= weaponMuzzleOrigin + projectileDist * idMath::RSqrt( projectileDist ) * owner->GetRenderEntity()->axis[2];
}
...
dnBeamSpeedTracer::dnBeamSpeedTracer( idEntity *owner, const float speed, const float distance, const idVec3 &muzzleOrigin, const idMat3 &tracerAxis ) : dnSpeedTracer( owner ) {
...
void dnRailBeam::Create( const idVec3 &beamEnd ) {
	...
	if ( smokeParticle != NULL ) {
		...
		if ( nSmokeParticles > 100 ) { nSmokeParticles = 100; }		// ivan - fix for performance
...
void dnRailBeam::Think( void )  {
	...
	if ( nSmokeParticles > 0 ){ 
		for ( int i=0; i < nSmokeParticles; i++ ) {
			if ( !gameLocal.smokeParticles->EmitSmoke( smokeParticle, smokeStartTime, gameLocal.random.RandomFloat(), smokeOffset - smokeLength * ( i + 1 ) * renderEntity.axis[2], renderEntity.axis, owner->timeGroup ) ) {
				deadSmokeCount++;
			}	
		}
	}

...
idCVar g_fov(						"g_fov",						"90",					CVAR_GAME | CVAR_ARCHIVE | CVAR_INTEGER | CVAR_NOCHEAT, "sets the player's field of view" );
...
idCVar af_useBodyDensityBuoyancy(	"af_useBodyDensityBuoyancy",	"0",					CVAR_GAME | CVAR_BOOL | CVAR_ARCHIVE, "Uses density of each body to calculate buoyancy." );
idCVar af_useFixedDensityBuoyancy(	"af_useFixedDensityBuoyancy",	"1",					CVAR_GAME | CVAR_BOOL | CVAR_ARCHIVE, "If set, use liquidDensity as a fixed density for each body when calculating buoyancy. If clear, bodies are floated uniformly by a scalar liquidDensity as defined in the decls." );

idCVar rb_showBuoyancy(				"rb_showBuoyancy",				"0",					CVAR_GAME | CVAR_BOOL | CVAR_TOOL, "Show rigid body buoyancy information." );

idCVar ai_think(					"ai_think",						"1",					CVAR_BOOL | CVAR_TOOL, "Prevent AI from thinking. For testing." );

idCVar actor_noDamage(				"actor_noDamage",				"0",					CVAR_BOOL | CVAR_TOOL, "Prevent actors from taking damage. For testing." );

idCVar g_testStuff(					"g_testStuff",					"0",					CVAR_INTEGER | CVAR_TOOL, "Does nothing, it's here just for testing various stuff." );

idCVar g_testModelRoll(				"g_testModelRoll",				"0",					CVAR_FLOAT | CVAR_TOOL, "Roll angles of the testModel." );
idCVar g_testModelPitch(			"g_testModelPitch",				"0",					CVAR_FLOAT | CVAR_TOOL, "Pitch angles of the testModel." );
idCVar g_testModelYaw(				"g_testModelYaw",				"0",					CVAR_FLOAT | CVAR_TOOL, "Yaw angles of the testModel." );

idCVar g_showModelNames(			"g_showmodelnames",				"0",					CVAR_BOOL | CVAR_TOOL, "Show model filenames." );
idCVar g_maxShowDistanceWorld(		"g_maxShowDistanceWorld",		"256",					CVAR_FLOAT | CVAR_GAME | CVAR_ARCHIVE, "Distance at which bboxes stop being shown." );

idCVar g_weaponDepthHack(			"g_weaponDepthHack",			"1",					CVAR_BOOL | CVAR_GAME | CVAR_ARCHIVE, "Crunch z depth on weapons" );
idCVar g_weaponFlashShadows(		"g_weaponFlashShadows",			"1",					CVAR_BOOL | CVAR_GAME | CVAR_ARCHIVE, "Cast shadows from weapons muzzleflashes." );

idCVar g_playerOverride(			"g_playerOverride",				"0",					CVAR_BOOL | CVAR_ARCHIVE, "Overrides the automatic player selection." );
idCVar g_player(					"g_player",						"0",					CVAR_GAME | CVAR_ARCHIVE | CVAR_INTEGER, "Changes the player\n"
																												"0 = player_default\n"
																												"1 = player_doommarine\n"
																												"2 = player_doommarine_d3xp\n"
																												"3 = player_doommarine_d3le\n" );

idCVar g_grabberProjDamageScale(	"g_grabberProjDamageScale",		"4.0",					CVAR_GAME | CVAR_FLOAT | CVAR_CHEAT, "Grabbed projectiles damage scale." );
idCVar g_gibVelocity(				"g_gibVelocity",				"1",					CVAR_GAME | CVAR_FLOAT | CVAR_ARCHIVE, "Velocity of the gibs." );
idCVar g_gibLifeTime(				"g_gibLifeTime",				"5",					CVAR_GAME | CVAR_FLOAT | CVAR_ARCHIVE, "Time before the gibs are removed." );
...
idCVar r_hdrMaxColorIntensity(		"r_hdrMaxColorIntensity",		"1.0",					CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "Adjusting this value will cause color burnout in a controllable fashion (range 0.0 - 256.0) \nHigher values reduces the contrast, lower values increases the contrast." );
idCVar r_hdrGammaCorrection(		"r_hdrGammaCorrection",			"1.0",					CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "Gamma Correction." );
idCVar r_hdrBloomIntensity(			"r_hdrBloomIntensity",			"0.75",					CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "Adjusts the bloom intensity. 0.0 disables both bloom as well as halo passes." );
idCVar r_hdrHaloIntensity(			"r_hdrHaloIntensity",			"1.0",					CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "Adjusts the halo intensity. Set to 0.0 disables halo pass." );
idCVar r_hdrEyeAdaptationRate(		"r_hdrEyeAdaptationRate",		"2",					CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "Total time in second taken to adjust eye exposure. Works only when automatic eye exposure is on." );
idCVar r_hdrEyeAdaptationBias(		"r_hdrEyeAdaptationBias",		"0.09",					CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "Automatic eye exposure factor (range 0 to 1) \n e.g. 0.0 - No automatic eye exposure, \n 0.5 - 50% automatic eye exposure, \n 1.0 Full automatic eye exposure." );
idCVar r_hdrEyeAdaptationBloomBias(	"r_hdrEyeAdaptationBloomBias",	"1",					CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "Bloom can have a separate eye adaptation bias. See cvar descritption for r_hdrEyeAdaptationBias." );
idCVar r_hdrBrightPassThreshold(	"r_hdrBrightPassThreshold",		"1.7",					CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "Brightness threshold for bright-pass. Works only when automatic exposure is on." );
idCVar r_hdrBrightPassOffset(		"r_hdrBrightPassOffset",		"5.0",					CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "Brightness offset for bright pass. Works only when automatic exposure is on." );
idCVar r_hdrMinLuminance(			"r_hdrMinLuminance",			"0.003",				CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "Luminance is restricted to this value to control whiteness when in pitch dark areas. Works only when automatic exposure is on." );
idCVar r_hdrMaxLuminance(			"r_hdrMaxLuminance",			"0.22",					CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "Luminance is restricted to this value to control darkness when in super-bright areas. Works only when automatic exposure is on." );
idCVar r_hdrLumUpdateRate(			"r_hdrLumUpdateRate",			"11",					CVAR_RENDERER | CVAR_ARCHIVE | CVAR_INTEGER, "Updates luminance data after every these many number of frames." );												
idCVar r_hdrVignetteBias(			"r_hdrVignetteBias",			"0.45",					CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "Vignette: Fading-of-edges-of-the-screen effect(range 0 to 1)" );												
idCVar r_hdrDebugMode(				"r_hdrDebugMode",				"0",					CVAR_TOOL | CVAR_INTEGER, "Shows all the textures generated for HDR: \n 1: Shows all textures \n 2: Decodes and shows all textures" );
idCVar r_hdrDebugTextureIndex(		"r_hdrDebugTextureIndex",		"0",					CVAR_TOOL | CVAR_INTEGER, "Show intermediate textures used for HDR: \n 1: scene Image(64x Scaled down) \n 2: Average Luminance Initial pass texture\n 3: Average luminance" );
...
idCVar pm_abilityModifierPassive(	"pm_ability_passive",			"0",					CVAR_GAME | CVAR_ARCHIVE | CVAR_INTEGER, "Changes the passive ability modifier\n"
																												"0 = None\n"
																												"1 = Invincibility\n"
																												"2 = Cloaking\n"
																												"3 = Super Strength\n"
...
// dashing
idCVar pm_maxDashes(				"pm_maxDashes",					"2",					CVAR_GAME | CVAR_INTEGER, "Max dashes the player can perform before having to recharge." );
idCVar pm_dashVelocity(				"pm_dashVelocity",				"5000",					CVAR_GAME | CVAR_INTEGER, "Velocity of the dash." );
idCVar pm_dashRechargeRate(			"pm_dashRechargeRate",			"25",					CVAR_GAME | CVAR_FLOAT, "Energy recharge rate when dashing is used" );

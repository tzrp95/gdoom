...
void idAI::Event_MoveToCover( void ) {
	...
	if ( enemyEnt == NULL || !MoveToCover( enemyEnt, lastVisibleEnemyPos ) ) {
		return;
	}
}
...
void idAI::Event_GetCombatNode( void ) {
	...
	if ( enemyEnt == NULL || !EnemyPositionValid() ) {
...
void idAI::Event_CanHitEnemy( void ) {
	...
	idActor *enemyEnt = enemy.GetEntity();
	if ( !AI_ENEMY_VISIBLE || enemyEnt == NULL ) {
		idThread::ReturnInt( false );
		return;
	}
...
void idAI::Event_ClearBurn( void ) {
	...
	if ( head.GetEntity() ) {
		head.GetEntity()->GetRenderEntity()->noShadow = spawnArgs.GetBool( "noshadows" );
...
=====================
*/
void idAI::Event_LaunchHomingMissile( void ) {
	idVec3				start;
	trace_t				tr;
	idBounds			projBounds;
	const idClipModel	*projClip;
...
=====================
*/
void idAI::Event_SetHomingMissileGoal( void ) {
	idActor *enemy = GetEnemy();
